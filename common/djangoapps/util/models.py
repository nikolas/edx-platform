...
def decompress_string(s):  # pylint: disable=invalid-name
    """
...
class CompressedTextField(models.TextField):
    """ TextField that transparently compresses data when saving the the database, and decompresses the data
    when retrieving it from the database. """

    COMPRESSED_ID = 'compressed'
    ...
    def get_prep_value(self, value):
        ...
        if value is not None:
            ...
            value = self.COMPRESSED_ID + value.encode('base64').decode('utf8')
        return value

    def to_python(self, value):
        if isinstance(value, unicode) and value.startswith(self.COMPRESSED_ID):
            value = value.replace(self.COMPRESSED_ID, '', 1)
